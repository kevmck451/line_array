...
if __name__ == '__main__':
    ...
    for filter in filters:
        ...
        process = True
        processes = {'nr':std, 'lp': filter}

        filenames = ['Test1_RAW-ch1', 'Test1_B12', 'Test1_BEns']
        plot_titles = [f'Raw Ch1 - LP: {filter}_5x - NR{std}',
                       f'Beamed 12 Mics - LP: {filter}_5x - NR{std}',
                       f'Beam Mixture - LP: {filter}_5x - NR{std}']
        save_tags = [f'RawCh1_LP{filter}_5x_NR{std}', f'BF12m_LP{filter}_5x_NR{std}', f'BMix_LP{filter}_5x_NR{std}']
