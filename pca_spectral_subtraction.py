...
class PCA_Calculator:
    def __init__(self,
        ...
        self.av_spectrum_cal, self.frequency_bins_cal = average_spectrum(audio_cal, display=False)
    ...
    def process_chunk(self, chunk):
        ...
        for i in range(16, 8, -1):
            ...
            try:
                print(f'Time Data Shape: {chunk.shape}')
                ...
                print(f'STFT Size: {stft_matrix.shape}')

                av_spec, freq_bins = average_spectrum(chunk, display=False)
                print(f'Av Spec Size: {av_spec.shape}')
                print(f'Freq Bin Size: {freq_bins.shape}')
...
def average_spectrum_ss(audio_object, fft_size=65536, overlap=0.5, **kwargs):
...
def create_plot(filename, plot_title, save_tag, threshold, process, processes):
    ...
    targets = [
        75, 118, 161, 203, 246, # tank
        290, 333, 377, 421, 464, # truck
        637, 681, 724, 767, 810,
